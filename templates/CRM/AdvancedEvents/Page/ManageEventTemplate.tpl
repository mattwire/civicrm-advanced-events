      ...
      <table id="options" class="display">
        ...
        {foreach from=$rows item=row}
          <tr id='rowid{$row.id}' class="{cycle values="odd-row,even-row"} crm-event crm-event_{$row.id}">
              <td class="crm-event-template_title">
                <a class="action-item crm-hover-button" href="{crmURL p="civicrm/event/manage/settings" q="action=update&id=`$row.id`&reset=1"}">{$row.template_title}</a>
              </td>
